...
class SemanticVisitor(NodeVisitor):
    ...
    def visit_Print(self, _print): #necesario
        ...
        if(node.value.return_type[0]!='string'):#???
    ...
    def visit_Read(self, read): #necesario
        if not(self.actual_t.find(read.value)):
            error(write.lineno, 'Error, Identifier not found in Read statement', filename=sys.argv[1])
